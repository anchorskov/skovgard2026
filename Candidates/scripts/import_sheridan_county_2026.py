# Candidates/scripts/import_sheridan_county_2026.py
#!/usr/bin/env python3
"""Generate INSERT SQL for Sheridan County 2026 race and candidate data.

Usage:
    python3 Candidates/scripts/import_sheridan_county_2026.py > /tmp/sheridan_county_insert.sql

Requires: Candidates/db/migrations/0004_offices_expand.sql applied first.
Source CSVs: /tmp/sheridan_county_2026/
"""
import csv
import hashlib
import re
...
CSV_DIR = "/tmp/sheridan_county_2026"
...
def derive_scope_kind(row):
    ...
    party = normalize_party(row.get("party"), row.get("ballot_party"))

    if office_level in {"municipal", "city"}:
        return "municipal"
    if office_level in {"precinct", "precinct committee", "precinct_committee"} or precinct:
...
def municipality(row):
    ...
    raw = re.sub(r"^(Town|City) of\s+", "", raw, flags=re.I)
    return raw or None
...
def city_from_address_or_row(row):
    address = (row.get("mailing_address") or "").lower()
    for city in KNOWN_CITIES:
        if re.search(rf"\b{re.escape(city.lower())}\b", address):
            return city
    return municipality(row)


def main():
    race_rows = load_csv(f"{CSV_DIR}/sheridan_county_2026_races.csv", RACE_HEADERS)
    candidate_rows = load_csv(f"{CSV_DIR}/sheridan_county_2026_candidates.csv", CANDIDATE_HEADERS)
    ...
    for row in candidate_rows:
        race_id = row["race_id"]
        party = normalize_party(row["party"], row["ballot_party"])
        full_name = row["candidate_name"]
        slug = candidate_slug(row)
        city = city_from_address_or_row(row)
