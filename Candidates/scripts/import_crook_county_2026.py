# Candidates/scripts/import_crook_county_2026.py
#!/usr/bin/env python3
"""Generate INSERT SQL for Crook County 2026 race and candidate data.

Usage:
    python3 Candidates/scripts/import_crook_county_2026.py > /tmp/crook_county_insert.sql

Requires: Candidates/db/migrations/0004_offices_expand.sql applied first.
Source CSVs: /tmp/crook_county_2026/
"""
import csv
import hashlib
import re
...
def derive_scope_kind(row):
    ...
    gender = infer_committee_gender(row) or (row.get("gender") or "").strip()
    party = normalize_party(row.get("source_party_text"), row.get("ballot_party"))

    if office_level in {"municipal", "city"}:
        return "municipal_precinct_group" if district_type == "precinct_group" else "municipal"
...
def municipality(row):
    if normalize_level(row.get("office_level")) != "city":
        return None
    raw = (row.get("jurisdiction_type") or "").strip()
    raw = re.sub(r"^(City|Town) of\s+", "", raw, flags=re.I)
    return raw or None
...
def slug_suffix(candidate_id):
    tail = (candidate_id or "").strip().split("--")[-1]
    if tail and tail != candidate_id:
        return hashlib.md5(candidate_id.encode()).hexdigest()[:8]
    return (candidate_id or "").strip().split("-")[-1] or hashlib.md5(candidate_id.encode()).hexdigest()[:8]
...
def main():
    race_rows = load_csv(f"{CSV_DIR}/crook_county_2026_races.csv", RACE_HEADERS)
    candidate_rows = load_csv(f"{CSV_DIR}/crook_county_2026_candidates.csv", CANDIDATE_HEADERS)
    ...
    for row in candidate_rows:
        ...
        committee_gender = infer_committee_gender(row) or normalize_gender(row["gender"])
