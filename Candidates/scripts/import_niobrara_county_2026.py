# Candidates/scripts/import_niobrara_county_2026.py
#!/usr/bin/env python3
"""Generate INSERT SQL for Niobrara County 2026 race and candidate data.

Usage:
    python3 Candidates/scripts/import_niobrara_county_2026.py > /tmp/niobrara_county_insert.sql

Requires: Candidates/db/migrations/0004_offices_expand.sql applied first.
Source CSVs: /tmp/niobrara_county_2026/
"""
import csv
import hashlib
import re
...
CSV_DIR = "/tmp/niobrara_county_2026"
...
def main():
    race_rows = load_csv(f"{CSV_DIR}/niobrara_county_2026_races.csv", RACE_HEADERS)
    candidate_rows = load_csv(f"{CSV_DIR}/niobrara_county_2026_candidates.csv", CANDIDATE_HEADERS)
    ...
    for sort_order, row in enumerate(race_rows, start=1):
        ...
        print(
            ...
            f"{q(title)}, {q(level)}, {q(district)}, {q(county)}, NULL, "
