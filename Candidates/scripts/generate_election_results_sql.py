...
def emit_sources(rows, out, source_role, supersedes=None):
    supersedes = supersedes or {}
    for r in dedup(rows, ["source_key"]):
        old_source_key = supersedes.get(r["source_key"])
        if old_source_key:
            out.append(
                "INSERT OR IGNORE INTO election_sources "
                "(source_key, election_id, county, source_role, endpoint_url, "
                "supersedes_source_id, status) SELECT "
                f"{sql_str(r['source_key'])}, "
                f"(SELECT id FROM election_events WHERE election_key = {sql_str(r['election_key'])}), "
                f"{sql_str(r['county'])}, {sql_str(source_role)}, {sql_str(r['source_url'])}, "
                f"id, 'active' FROM election_sources WHERE source_key = {sql_str(old_source_key)};"
            )
        else:
            out.append(
                "INSERT OR IGNORE INTO election_sources "
...
def parse_supersedes(values, source_keys):
    mappings = {}
    for value in values or []:
        if "=" not in value:
            raise ValueError(
                "--supersedes-source must be NEW_SOURCE_KEY=OLD_SOURCE_KEY"
            )
        new_source_key, old_source_key = value.split("=", 1)
        if not new_source_key or not old_source_key:
            raise ValueError(
                "--supersedes-source must include both source keys"
            )
        if new_source_key not in source_keys:
            raise ValueError(
                f"Superseding source key is not present in the input CSVs: {new_source_key}"
            )
        if new_source_key in mappings:
            raise ValueError(f"Duplicate superseding source key: {new_source_key}")
        mappings[new_source_key] = old_source_key
    return mappings
...
def main():
    ...
    p.add_argument(
        ...
        action="append",
        default=[],
        metavar="NEW_SOURCE_KEY=OLD_SOURCE_KEY",
        help=(
            "Record append-only source succession. Repeat for multiple input "
            "sources. The old source key must already exist when the SQL runs."
        ),
    )
    args = p.parse_args()
    ...
    supersedes = parse_supersedes(args.supersedes_source, source_keys)
    ...
    out.append("")
    emit_sources(rows, out, args.source_role, supersedes)
    out.append("")
