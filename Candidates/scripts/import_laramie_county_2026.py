# Candidates/scripts/import_laramie_county_2026.py
#!/usr/bin/env python3
"""Generate INSERT SQL for Laramie County 2026 race and candidate data.

Usage:
    python3 Candidates/scripts/import_laramie_county_2026.py > /tmp/laramie_county_insert.sql

Requires: Candidates/db/migrations/0004_offices_expand.sql applied first.
Source CSVs: /tmp/laramie_county_2026/
"""
import csv
import hashlib
import re
...
CSV_DIR = "/tmp/laramie_county_2026"
...
def derive_scope_kind(row):
    ...
    if office_level in {"municipal", "city"}:
        return "municipal_ward" if (row.get("ward") or "").strip() else "municipal"
...
def main():
    race_rows = load_csv(f"{CSV_DIR}/laramie_county_2026_races.csv", RACE_HEADERS)
    candidate_rows = load_csv(f"{CSV_DIR}/laramie_county_2026_candidates.csv", CANDIDATE_HEADERS)
    ...
    for row in candidate_rows:
        ...
        city = row["mailing_city"] or municipality(row)
        state = row["mailing_state"] or "WY"
        ...
        source_page = row["source_file"] or None
        external_candidate_id = row["candidate_id"]
        committee_gender = normalize_gender(row["gender"])
