# Candidates/scripts/import_goshen_county_2026.py
#!/usr/bin/env python3
"""Generate INSERT SQL for Goshen County 2026 race and candidate data.

Usage:
    python3 Candidates/scripts/import_goshen_county_2026.py > /tmp/goshen_county_insert.sql

Requires: Candidates/db/migrations/0004_offices_expand.sql applied first.
Source CSVs: /tmp/goshen_county_2026/
"""
import csv
import hashlib
import re
...
CSV_DIR = "/tmp/goshen_county_2026"
...
EMAIL_FIXES = {
    "jtiddings@gmail": "jtiddings@gmail.com",
    "jimplumbing700@gmail": "jimplumbing700@gmail.com",
}
...
def derive_scope_kind(row):
    ...
    gender = infer_committee_gender(row) or normalize_gender(row.get("gender"))
...
def municipality(row):
    if normalize_level(row.get("office_level")) != "city":
        return None
    return (row.get("jurisdiction_name") or "").strip() or None
...
def normalize_email(value):
    ...
    if not raw:
        return None
    lower = raw.lower()
    return EMAIL_FIXES.get(lower, raw)
...
def main():
    race_rows = load_csv(f"{CSV_DIR}/goshen_county_2026_races.csv", RACE_HEADERS)
    candidate_rows = load_csv(f"{CSV_DIR}/goshen_county_2026_candidates.csv", CANDIDATE_HEADERS)
