# Candidates/scripts/import_sweetwater_county_2026.py
#!/usr/bin/env python3
"""Generate INSERT SQL for Sweetwater County 2026 race and candidate data.

Usage:
    python3 Candidates/scripts/import_sweetwater_county_2026.py > /tmp/sweetwater_county_insert.sql

Requires: Candidates/db/migrations/0004_offices_expand.sql applied first.
Source CSVs: /tmp/sweetwater_county_2026/
"""
import csv
import hashlib
import re
...
CSV_DIR = "/tmp/sweetwater_county_2026"
...
def derive_scope_kind(row):
    ...
    if office_level in {"precinct", "precinct committee", "precinct_committee"}:
...
def main():
    race_rows = load_csv(f"{CSV_DIR}/sweetwater_county_2026_races.csv", RACE_HEADERS)
    candidate_rows = load_csv(f"{CSV_DIR}/sweetwater_county_2026_candidates.csv", CANDIDATE_HEADERS)
    ...
    for sort_order, row in enumerate(race_rows, start=1):
        ...
        county = row["county"] or "Sweetwater"
