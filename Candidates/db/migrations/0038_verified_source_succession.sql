-- Candidates/db/migrations/0038_verified_source_succession.sql
--
-- Excludes a superseded source from current-result selection once an active
-- successor has a verified snapshot. Source facts and snapshots remain
-- append-only and queryable. This closes the gap where a malformed historical
-- contest key could remain current because it did not collide with the
-- corrected contest key in a complete official successor.
...
WHERE esrc.county IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM election_sources successor
    JOIN v_election_latest_snapshots successor_snapshot
      ON successor_snapshot.source_id = successor.id
    WHERE successor.supersedes_source_id = esrc.id
