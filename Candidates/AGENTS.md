...
- **`Candidates/docs/natrona_seat_remediation_checklist.md`**: reviewed task and validation gates for correcting Natrona precinct committee seat counts, ingesting the official result successor, resolving unnamed write-in winners, and regenerating the outreach file. Read before any Natrona seat or precinct-person outcome work.
...
- **`Candidates/docs/election_results_schema.md`**: table relationships, append-only versus mutable-control-data boundaries, source-check lifecycle, snapshot-acceptance rule, source-precedence and verified-succession rules, and the safe localhost migration procedure for the election-results schema (migrations 0028-0038). Read before touching `election_*` tables/views, `extract_election_results_*.py`, `generate_election_results_sql.py`, `../Results/`, or `src/pages/results/`.
...
- **`Candidates/docs/final_results_primary.md`**: repeatable process for promoting a county from unofficial to official/certified final results, including archive, cross-verification, append-only official seeds, local validation, natural-key roster corrections, and the production hold. Generalized from the Natrona 2026-08-26 run; read before promoting any other county's official results.
